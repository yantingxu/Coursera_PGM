...
for m = 1:length(edgeFromIndx),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    clusterFrom = P.clusterList(i);
    clusterTo = P.clusterList(j);
    msgVars = intersect(clusterFrom.var, clusterTo.var);
    msgCards = zeros(1, length(msgVars));
    for k = 1:length(msgVars)
        msgVar = msgVars(k);
        idx = find(clusterFrom.var == msgVar);
        msgCards(k) = clusterFrom.card(idx);
    end
    MESSAGES(i, j).var = msgVars;
    MESSAGES(i, j).card = msgCards;
    MESSAGES(i, j).val = ones(1, prod(msgCards));
...
while (1),
    ...
    for k = 1:length(msgs)
        if idx(k) ~= j
            msg = msgs(k);
            factor = FactorProduct(factor, msg);
        end
    end
    mVars = setdiff(P.clusterList(i).var, prevMessage.var);
    factor = FactorMarginalization(factor, mVars);
    ...
    if mod(iteration, length(edgeFromIndx)) == 0
        if (CheckConvergence(MESSAGES, lastMESSAGES, iteration))
            break;
        end
