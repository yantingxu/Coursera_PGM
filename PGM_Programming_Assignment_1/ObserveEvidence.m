...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            invalidIdx = find(assignments(:, indx) ~= x);
            invalidAsgns = assignments(invalidIdx, :);
            invalidVals = zeros(1, length(invalidIdx));

            phi = SetValueOfAssignment(F(j), invalidAsgns, invalidVals);
            F(j) = phi;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
