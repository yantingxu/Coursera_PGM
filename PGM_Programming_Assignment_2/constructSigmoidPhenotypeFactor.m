...
for i = 1:16
    assignment = IndexToAssignment(i, [2 2 2 2]);
    w = 0.0;
    w += alleleWeights{1}(assignment(1));
    w += alleleWeights{1}(assignment(3));
    w += alleleWeights{2}(assignment(2));
    w += alleleWeights{2}(assignment(4));
    sw = computeSigmoid(w);

    idx = AssignmentToIndex([1 assignment], phenotypeFactor.card);
    phenotypeFactor.val(idx) = sw;
    idx = AssignmentToIndex([2 assignment], phenotypeFactor.card);
    phenotypeFactor.val(idx) = 1 - sw;
end
