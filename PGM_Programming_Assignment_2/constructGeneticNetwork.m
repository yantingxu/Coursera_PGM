...
for i = 1:numPeople
    ...
    father = parents(1);
    monther = parents(2);
    if father == 0 && monther == 0
        genotype = genotypeGivenAlleleFreqsFactor(alleleFreqs, i);
    else
        genotype = genotypeGivenParentsGenotypesFactor(numAlleles, i, father, monther);
    end
    phenotype = phenotypeGivenGenotypeFactor(alphaList, i, numPeople + i);
    factorList(i) = genotype;
    factorList(numPeople + i) = phenotype;
end
