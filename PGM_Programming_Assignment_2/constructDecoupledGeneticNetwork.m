...
for i = 1:numPeople
    ...
    if parents(1) == 0 && parents(2) == 0
        geneCopy1 = childCopyGivenFreqsFactor(alleleFreqs, i);
        geneCopy2 = childCopyGivenFreqsFactor(alleleFreqs, i+numPeople);
    else
        geneCopy1 = childCopyGivenParentalsFactor(numAlleles, i, parents(1), parents(1)+numPeople);
        geneCopy2 = childCopyGivenParentalsFactor(numAlleles, i+numPeople, parents(2), parents(2)+numPeople);
    end
    factorList(i) = geneCopy1;
    factorList(i+numPeople) = geneCopy2;
