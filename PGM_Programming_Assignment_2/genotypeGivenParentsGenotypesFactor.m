...
cardNum = numAlleles + numAlleles*(numAlleles-1)/2;
genotypeFactor.card = [cardNum, cardNum, cardNum];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
matrix = zeros(numAlleles, numAlleles);
for f = 1:cardNum
    for m = 1:cardNum
        fg = genotypesToAlleles(f, :);
        mg = genotypesToAlleles(m, :);
        for o = 1:length(fg)
            for p = 1:length(mg)
                childGenotype = allelesToGenotypes(fg(o), mg(p));
                idx = AssignmentToIndex([childGenotype, f, m], genotypeFactor.card);
                genotypeFactor.val(idx) += 0.25;
            end
        end
    end
end
